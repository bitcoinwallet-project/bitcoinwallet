...
#if defined(NDEBUG)
# error "Bitcoinwallet cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Bitcoinwallet Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s WBIT additional fees, %d delta bytes\n",
...
void ThreadScriptCheck() {
    RenameThread("bitcoinwallet-scriptch");
