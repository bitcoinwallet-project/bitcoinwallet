...
const char * const BITCOIN_CONF_FILENAME = "bitcoinwallet.conf";
const char * const BITCOIN_PID_FILENAME = "bitcoinwallet.pid";
...
#else
    const char* pszModule = "bitcoinwallet";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bitcoinwallet";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Bitcoinwallet";
#else
    // Unix
    return pathRet / ".bitcoinwallet";
#endif
#endif
}
